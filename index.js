var accessRolesError = require('./lib/accessRolesError');
var fs = require('fs');

function accessRoles(app) {
...
function setMiddlewares(app, options, callback) {
    for (var i = 0; i < options.roles.length; i++) {
        ...
        for (let j = 0; j < role.methods.length; j++) {
            var method = role.methods[j].toString();

            try {
                app[method.toLowerCase()](role.path, start(options, i));
            } catch (error) {
...
function start(options, roleIndex) {
    return function (req, res, next) {
        ...
        if(!user) throw new accessRolesError('Data source not found');
